...
echo "PHP version               : ${PHP72_VERSION}"
echo "Extra Extensions          : ${PHP72_EXTENSIONS}"
...
if [[ -z "${EXTENSIONS##*,ssh2,*}" ]]; then
    ...
    if [[ "$?" = "1" ]]; then
        ...
        echo "ssh2 requires PHP >= 7.0.0, installed version is ${PHP72_VERSION}"
    fi
fi
...
if [[ -z "${EXTENSIONS##*,protobuf,*}" ]]; then
    ...
    if [[ "$?" = "1" ]]; then
        ...
        echo "yar requires PHP >= 7.0.0, installed version is ${PHP72_VERSION}"
    fi
fi
...
if [[ -z "${EXTENSIONS##*,pdo_sqlsrv,*}" ]]; then
    ...
    if [[ "$?" = "1" ]]; then
        ...
        echo "pdo_sqlsrv requires PHP >= 7.1.0, installed version is ${PHP72_VERSION}"
    fi
fi
